...
(function () {
  ...
  const ICONS = {
    ...
    essentials:
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="4" width="14" height="17" rx="2"/><path d="M9 4V2h6v2M8 10h8M8 14h8"/></svg>',
    skincare:
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M9 3h6M10 3v4.2a3 3 0 0 1-.7 1.9L8 11.4A5 5 0 0 0 7 14.5V19a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2v-4.5a5 5 0 0 0-1-3.1l-1.3-2.3a3 3 0 0 1-.7-1.9V3"/></svg>',
    cleansing:
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M9 3h6M10 3v4.2a3 3 0 0 1-.7 1.9L8 11.4A5 5 0 0 0 7 14.5V19a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2v-4.5a5 5 0 0 0-1-3.1l-1.3-2.3a3 3 0 0 1-.7-1.9V3"/><path d="M18 6c1.2 1.5 1.8 2.5 1.8 3.2a1.8 1.8 0 1 1-3.6 0C16.2 8.5 16.8 7.5 18 6Z"/></svg>',
    moisturizing:
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3c3.2 3.8 4.8 6.5 4.8 8.5a4.8 4.8 0 0 1-9.6 0C7.2 9.5 8.8 6.8 12 3Z"/><path d="M8 20h8"/></svg>',
    treatments:
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18M3 12h18"/><circle cx="12" cy="12" r="7"/></svg>',
    "sun-care":
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3.5"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3M4.9 4.9 7 7M17 17l2.1 2.1M19.1 4.9 17 7M7 17l-2.1 2.1"/></svg>',
    masks:
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M6 4h12l-1 12a5 5 0 0 1-10 0L6 4Z"/><path d="M9 10h.01M15 10h.01M10 15c1.2.7 2.8.7 4 0"/></svg>',
    tools:
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="m6 3 4 4-6.5 6.5a3.5 3.5 0 0 0 5 5L15 12l4 4 2-2-4-4 3-3-3-3-3 3-4-4L6 3Z"/></svg>',
    "body-care":
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M9 3h6M10 3v4l-2 3v9a2 2 0 0 0 2 2h4a2 2 0 0 0 2-2v-9l-2-3V3"/><path d="M8 13h8"/></svg>',
    "hair-scalp":
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M7 21c-1-5 .5-9 4-11.5C14.5 7 17 5.5 17 2c3 4.5 1.6 9.7-2 12.2-2.5 1.8-3 4.4-2 6.8"/><path d="M8 4c1.5 2.6 1.8 4.6.7 6.3"/></svg>',
    "beauty-essentials":
      '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="m6 3 12 12M14 3l7 7M3 14l7 7"/><path d="m5 19 7-7 2 2-7 7H5v-2Z"/></svg>',
    star:
      ...
      <article class="product-card reveal" data-id="${product.id}" tabindex="0">
        ...
        <div class="product-body">
          ...
          <div class="product-actions">
            ...
            ${
              isValidProductUrl(product.amazonUrl)
                ? `
                  <a
                    class="btn-amazon"
  ...
  function isValidProductUrl(url) {
    return (
      typeof url === "string" &&
      /^https?:\/\//i.test(url) &&
      !url.includes("AMAZON_AFFILIATE_LINK_HERE")
    );
  }
  ...
  function renderFilterPanel() {
    ...
    const rest = counts
      .map(
        (c) => `
      <button class="filter-option ${
        state.category === c.slug ? "is-active" : ""
  ...
  function syncFilterUI() {
    document
      .querySelectorAll("#category-filter-list .filter-option")
      .forEach((btn) => {
  ...
  function renderFooter() {
    ...
    document.getElementById("footer-brand-name-bottom").textContent =
      siteConfig.brandName;
    ...
    socialWrap.innerHTML = Object.entries(siteConfig.social)
      .map(
        ([key, url]) =>
          `<a class="icon-btn" href="${url}" target="_blank" rel="noopener noreferrer" aria-label="${key}">
            ${socialIconFor(key)}
          </a>`
      )
      .join("");
  ...
  function initShowcase() {
    ...
    document.getElementById("sc-prev").addEventListener("click", (e) => {
      e.stopPropagation();
      go(-1);
      restartTimer();
    });

    document.getElementById("sc-next").addEventListener("click", (e) => {
      e.stopPropagation();
      go(1);
      restartTimer();
    });
    ...
    root.addEventListener("click", (e) => {
      if (e.target.closest(".sc-nav,.sc-dots")) return;
      openModal(scItems[scIdx].id);
    });
  ...
  function paintSlide(animate) {
    ...
    const img = hasProductImage ? p.image : "";
  ...
  function startTimer() {
    if (prefersReduced || scItems.length < 2) return;

    stopTimer();

    scTimer = setInterval(() => go(1), SHOWCASE_MS);
  }
  ...
  function openModal(id) {
    ...
    if (hasProductImage) {
      ...
      modalImg.alt = `${product.name} product image`;
    ...
    const amazonBtn =
      document.getElementById("modal-amazon");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    document.getElementById("year-attr").textContent =
      new Date().getFullYear();
